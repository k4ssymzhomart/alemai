...
def _h_objection_days(session: Session, lang: Lang, question: str) -> CopilotAnswerOut:
    ...
    txt = _pick(
        {
            "kk": f"№{obj.case_ref} іс бойынша қарсылыққа {d} жұмыс күні қалды (мерзім {date}). "
            "Үнсіздік — автоматты түрде алып тастау (п. 27).",
...
def _h_reconcile(session: Session, lang: Lang) -> CopilotAnswerOut:
    ...
    txt = _pick(
        {
            "kk": f"{n} жазба (≈{fmt_tenge(amt)}) көрсетілген, бірақ шот-тізілімге енгізілмеген — "
            "жоғалған кіріс. «Салыстыру» экранынан қараңыз.",
...
def _h_monitoring_types(lang: Lang) -> CopilotAnswerOut:
    txt = _pick(
        {
            "kk": "Мониторинг екі түрге бөлінеді: ағымдағы (әр жұмыс күні ИСФ-те) және "
            "жоспардан тыс. Проактивный/целевой түрлері 2022–2024 редакцияларда болған, "
            "қазір жоқ.",
...
def _h_proactive(lang: Lang) -> CopilotAnswerOut:
    txt = _pick(
        {
            "kk": "Проактивный мониторинг 2022–2024 жылдардағы редакцияларда болды, қазір "
            "жойылған. Пациенттерге сауалнама тәсіл ретінде қалды (п. 6 пп. 5).",
...
def _h_refuse(lang: Lang) -> CopilotAnswerOut:
    txt = _pick(
        {
            "kk": "Мен ойдан цифр құрастырмаймын. Барлық сандар жүйеде деректерден есептеледі — "
            "тілдік модель арқылы өтпейді.",
